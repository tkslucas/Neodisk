...
//  navigate-away demotion left running), plus its hover help.
//

import SwiftUI
import NeodiskKit

/// Content of a sidebar row's background-scan hover help, independent of any
...
/// shows the live percent and item count in a native macOS help tooltip, which
/// the system keeps clear of the narrow row and the sidebar's window edges.
struct SidebarScanBar: View {
    ...
    var body: some View {
        StripedProgressBar(value: progress.metrics.progressFraction)
            ...
            .help(
                Text("Scanning…")
                    + Text(verbatim: "\n\(tooltipData.detailText)")
            )
    }

    private var tooltipData: SidebarScanTooltipData {
        SidebarScanTooltipData(
